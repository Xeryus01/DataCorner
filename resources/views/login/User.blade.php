...
<head>
    ...
    <title>Login — Datapedia BPS</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .login-wrap {
            ...
            padding: 2rem 1rem; background: #f1f5f9;
        }
        .login-card {
        ...
        .card-header {
            background: #002B6A; padding: 1.75rem 1.75rem 1.25rem;
            display: flex; flex-direction: column; align-items: flex-start; gap: 12px;
            position: relative; overflow: hidden;
        }
        .card-header::before {
            content: ''; position: absolute; top: -40px; right: -40px;
            width: 140px; height: 140px; border-radius: 50%; background: rgba(31,111,214,0.25);
        }
        .card-header::after {
            content: ''; position: absolute; bottom: -30px; right: 60px;
            width: 80px; height: 80px; border-radius: 50%; background: rgba(74,155,255,0.15);
        }
        ...
        .field-box {
            ...
            border-radius: 8px; background: #f8fafc; transition: border-color 0.15s, box-shadow 0.15s, background 0.15s; overflow: hidden;
        }
        .field-box:focus-within { border-color: #1F6FD6; box-shadow: 0 0 0 3px rgba(31,111,214,0.1); background: #fff; }
        .field-prefix {
            display: flex; align-items: center; gap: 6px; padding: 0 12px;
            border-right: 1px solid #e2e8f0; background: #f1f5f9; color: #94a3b8;
            font-size: 13px; font-weight: 600; white-space: nowrap; user-select: none; flex-shrink: 0;
        }
        ...
        .field-input {
            flex: 1; min-width: 0; height: 44px; padding: 0 12px; border: none;
            background: transparent; color: #0f172a; font-size: 14px; font-family: inherit; outline: none;
        }
        .field-input::placeholder { color: #cbd5e1; font-size: 13px; }
        .eye-btn {
            display: flex; align-items: center; padding: 0 12px; background: none;
            border: none; color: #94a3b8; cursor: pointer; flex-shrink: 0; transition: color 0.15s;
        }
        ...
        .alert-box {
            font-size: 13px; padding: 10px 12px; border-radius: 8px; margin-bottom: 1rem;
        ...
        .link { font-size: 13px; color: #1F6FD6; font-weight: 500; text-decoration: none; cursor: pointer; }
        .link:hover { text-decoration: underline; }
        .btn-submit {
            width: 100%; height: 44px; background: #002B6A; color: #fff; border: none;
        ...
        .register-row { text-align: center; font-size: 13px; color: #64748b; }
        .register-row .link { font-weight: 600; }
        .badge-secure {
            display: flex; align-items: center; justify-content: center; gap: 6px;
            background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 20px;
            padding: 6px 14px; font-size: 11px; color: #94a3b8; margin-top: 1rem;
        }
...
<body>
    <div class="login-wrap">
        <div class="login-card">
            <div class="card-header">
                <div class="logo-row">
                    <div class="logo-box">
                        <img src="{{ asset('image/logo-pst.png') }}" alt="Logo">
                    </div>
                    <span class="logo-name">Datapedia · BPS Babel</span>
                </div>
                <div class="header-title">Selamat datang</div>
                <div class="header-sub">Masuk untuk mengakses layanan statistik</div>
            </div>
                ...
                <form action="{{ route('prosesloginUser') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="field-box">
                            <div class="field-prefix">
                                <i class="ti ti-device-mobile"></i> +62
                            </div>
                            <input type="tel" class="field-input" name="no_hp" id="no_hp"
                                   placeholder="81234567890" autocomplete="tel"
                                   value="{{ old('no_hp') }}" required>
                        </div>
                        <p class="hint-text">Masukkan nomor tanpa awalan 0 atau 62</p>
                    ...
                    <div class="form-group">
                        ...
                        <div class="field-box">
                            ...
                            <button type="button" class="eye-btn" onclick="togglePass('password','eyeIcon')" aria-label="Toggle password">
                    ...
                    <button type="submit" class="btn-submit" id="submitBtn">
                        <i class="ti ti-login" style="font-size:16px"></i> <span id="btnText">Masuk</span>
                    </button>
                    ...
                    <div class="register-row">
                        Belum punya akun? <a href="{{ route('registerUser') }}" class="link">Daftar sekarang</a>
                    </div>

                    <div class="badge-secure">
                        <i class="ti ti-shield-check"></i> Koneksi aman · SSL terenkripsi
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        Swal.fire({ icon: 'warning', title: 'Sesi Berakhir', text: 'Silakan login kembali.', confirmButtonText: 'Login Ulang', confirmButtonColor: '#002B6A' })
            .then(() => { window.location.href = "{{ route('loginUser') }}"; });
    </script>
    @endif
    ...
    <script>
        ...
        form.addEventListener('submit', () => {
            ...
            btn.disabled = true; btn.style.background = '#1F6FD6';

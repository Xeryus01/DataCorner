...
@section('content')
<x-admin.page-header title="Edit Rekomendasi Statistik" subtitle="Ubah data pengunjung layanan rekomendasi statistik" :breadcrumbs="['Datapedia','Statistik','Rekomendasi','Edit']" />
@php [$tahun, $bulan] = explode('-', $data->periode); @endphp
<div class="card" style="max-width:600px">
    <div class="card-header"><div class="card-header-left"><div class="card-title"><i class="ti ti-bulb"></i>Form Edit Rekomendasi</div></div></div>
    <div class="card-body">
        @if($errors->any())<div class="form-error">@foreach($errors->all() as $e)<div>• {{$e}}</div>@endforeach</div>@endif
        <form action="{{ route('statistik.rekomendasi.update', $data->id) }}" method="POST">
            @csrf @method('PUT')
            ...
            @php $fields=[['name'=>'survei','label'=>'Survei'],['name'=>'kompromin','label'=>'Kompromin'],['name'=>'opd','label'=>'OPD'],['name'=>'instansi','label'=>'Instansi']]; @endphp
            ...
            <div class="form-actions"><button type="submit" class="btn-primary"><i class="ti ti-device-floppy"></i>Update</button><a href="{{ route('statistik.rekomendasi.index') }}" class="btn-ghost">Batal</a></div>
        </form>
    </div>
</div>
@endsection
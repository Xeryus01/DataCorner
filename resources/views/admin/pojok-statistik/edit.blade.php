...
@section('content')
<x-admin.page-header title="Edit Pojok Statistik" subtitle="Ubah data pengunjung Pojok Statistik" :breadcrumbs="['Datapedia','Statistik','Pojok','Edit']" />
@php [$tahun, $bulan] = explode('-', $data->periode); @endphp
<div class="card" style="max-width:600px">
    <div class="card-header"><div class="card-header-left"><div class="card-title"><i class="ti ti-map-pin"></i>Form Edit Pojok Statistik</div></div></div>
    <div class="card-body">
        @if($errors->any())<div class="form-error">@foreach($errors->all() as $e)<div>• {{$e}}</div>@endforeach</div>@endif
        <form action="{{ route('statistik.pojok-statistik.update', $data->id) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="form-actions"><button type="submit" class="btn-primary"><i class="ti ti-device-floppy"></i>Update</button><a href="{{ route('statistik.pojok-statistik.index') }}" class="btn-ghost">Batal</a></div>
        </form>
    </div>
</div>
@endsection
...
@section('content')
<x-admin.page-header title="Tambah Informasi Magang" subtitle="Input data informasi program magang" :breadcrumbs="['Datapedia','Magang','Informasi','Tambah']" />
<div class="card" style="background:#fff;border:0.5px solid #e2e8f0;border-radius:12px;overflow:hidden;max-width:640px">
    <div style="padding:14px 20px;border-bottom:0.5px solid #e2e8f0"><div style="font-size:13px;font-weight:600;color:#0f172a;display:flex;align-items:center;gap:8px"><i class="ti ti-megaphone" style="font-size:16px;color:#1F6FD6"></i>Form Tambah Informasi Magang</div></div>
    <div style="padding:20px">
        ...
        @endif
        <form method="POST" action="{{route('admin_informasi-magang.store')}}" enctype="multipart/form-data">
            @csrf
            ...
            <div style="display:flex;gap:8px">
                ...
                <a href="{{route('admin_informasi-magang.index')}}" style="display:inline-flex;align-items:center;gap:6px;padding:8px 20px;background:#fff;color:#64748b;border:0.5px solid #e2e8f0;border-radius:8px;font-size:13px;font-weight:500;text-decoration:none">Batal</a>
            </div>
        </form>
    </div>
</div>

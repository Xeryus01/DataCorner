    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 bg-white rounded-3xl shadow-md">
          ...
          @if ($pendaftaran->status === 'diterima')
            <div class="bg-primary rounded-2xl p-6 border border-primary">
              ...
              <div class="flex flex-col items-center gap-3">
                <!-- HANYA DI TAMPILKAN PADA WEBVIEW SAJA WEB TIDAK PERLU -->
                <!-- <a href="{{ route('daftar-magang.presensi', $pendaftaran->id) }}" class="w-full max-w-xs">
                  <button aria-label="Presensi Magang" type="button"
                    class="w-full bg-white/20 text-white text-xs md:text-base font-semibold py-3 px-8 rounded-xl hover:bg_white/10 hover:bg-white/10 flex items-center justify-center">Presensi
                    Magang</button>
                </a> -->
                <a href="{{ route('daftar-magang.log-harian') }}" class="w-full max-w-xs">
                  <button aria-label="Log Harian Magang" type="button"
                    class="w-full bg-white/20 text-white text-xs md:text-base font-semibold py-3 px-8 rounded-xl hover:bg-white/10 flex items-center justify-center">Log
      ...
      @else
        ...
        <form action="{{ route('daftar-magang.store') }}" method="POST" enctype="multipart/form-data"
          ...
          {{-- Magang Details Card --}}
          <div class="bg-gradient-to-r from-yellow-50 to-yellow-100 rounded-2xl p-6 border border-yellow-100 mt-8">
            <div class="flex items-center mb-6">
              <div class="w-10 h-10 bg-yellow-600 rounded-xl flex items-center justify-center mr-3">
                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="mt-6 px-6">
              <div class="flex items-center mb-2">
                ...
                <x-input-label for="wilayah_bps_id" :value="__('Wilayah Magang')" />
              </div>
              <select name="wilayah_bps_id" id="wilayah_bps_id" class="w-full px-4 py-3 rounded-xl bg-white/80 border focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                  <option value="">-- Pilih Wilayah Magang --</option>
              ...
              <x-input-error :messages="$errors->get('wilayah_bps_id')" class="mt-2" />
            </div>
          </div>
          ...
          <div class="mt-6 px-6">
            <div class="flex items-center mb-2">
              ...
              <x-input-label value="Status Difabel" />
            </div>

            <label class="flex items-center space-x-3 cursor-pointer">
              <input type="checkbox" id="is_difabel" name="is_difabel" value="1" class="w-5 h-5 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500"
                {{ old('is_difabel', $data->is_difabel ?? false) ? 'checked' : '' }} >
              <span class="text-gray-900 font-medium">
                Peserta merupakan penyandang disabilitas (difabel)
              </span>
            </label>

            <p class="text-sm text-gray-500 mt-1">
              Centang jika peserta memiliki kebutuhan atau layanan khusus.
            </p>

            <x-input-error :messages="$errors->get('is_difabel')" class="mt-2" />
          </div>
